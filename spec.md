...
• Shell text is read with a parser that cannot be made to recurse: the bash grammar is table-driven, and its tree is walked with an explicit stack, since text nested 50,000 deep builds a tree 50,000 deep. Following that text — $(...), backquotes, eval, sh -c text, wrappers — shares one budget of 16 levels, and past it, or wherever the text does not parse, the program reads as unresolvable rather than as whatever parsed. One command line lists at most 32 programs.
...
PAM
/etc/pam.d/*, and /usr/lib/pam.d/* for a service /etc/pam.d does not name — one entry per rule line, its module the target: a bare name resolved in libpam's module directory (the standard one holding pam_permit.so, so a copy planted in an unused /lib/security is not taken for it), so every module a stack loads is checked against its package and a replaced pam_unix.so is PackagedModified. A module that is not installed is noted and runs nothing. pam_exec lines target the program they run; a module outside the standard directories is NonStandardLocation
polkit
...
inetd and xinetd
The programs a network super-server starts, one per connection, often as root; the listener is a systemd unit and reported as one. Read only where the daemon is installed. xinetd: /etc/xinetd.conf, its defaults block, include and includedir, read as xinetd 2.3.15 reads them: an included directory in strcmp order, skipping any name containing a . or ending in ~, so telnet.bak never loads. One entry per service, off where disable = yes or the defaults' enabled or disabled list says so, with server and server_args as the command, user as the principal, and INTERNAL and redirect services naming no program. += and -= apply only to list attributes. inetd (openbsd-inetd, inetutils-inetd): one entry per /etc/inetd.conf line, whose argv includes argv[0], so a tcpd line resolves through the wrapper to the daemon tcpd runs. Fedora packages neither
sudo plugins
...
Reproduced { by }
No package's manifest vouches for the file, but it is byte for byte what the named generator makes from files that are packaged and intact, regenerated here the way the generator does it. As trustworthy as those files, and treated as packaged and intact from then on. See Reproduced files below.
Unpackaged
...
Reproduced files. Some configuration is written by a package's own tooling, so no manifest holds its digest, and every Debian-family or Fedora host would otherwise show it as unverified. Where the generator is deterministic it is re-run here, as a port of its own code, from its inputs, and the file is Reproduced only when every byte matches and every input is packaged and intact. Any other byte, an added auth sufficient pam_permit.so included, leaves the file with the verdict it had. A package database's verdict stands, except a %ghost file's, which has no digest to stand on.
• /etc/nsswitch.conf on Debian, Ubuntu and Mint: libc-bin's postinst copies /usr/share/libc-bin/nsswitch.conf.
• /etc/pam.d/common-* on Debian, Ubuntu and Mint: pam-auth-update (libpam-runtime 1.5) assembles each from the packaged template in /usr/share/pam and the profiles in /usr/share/pam-configs, in the order and selection its saved state in /var/lib/pam records, with its line merging reproduced exactly (every end in the control and module rewritten to the jump count, a line with no options ending in a space). A stack regenerated from a different selection of packaged profiles, one disabled with pam-auth-update --disable, is still Reproduced: the selection is the administrator's, made through the tool.
• /etc/pam.d/system-auth and the other five authselect stacks on Fedora: authselect 1.7 renders each from the profile template in /usr/share/authselect/vendor or default, with the features authselect.conf lists, its template operators and expression evaluator ported and checked against authselect's own output. A custom profile lives in /etc and is nobody's package. authselect's nsswitch.conf merges user maps and is not reproduced; §8's NSS rule covers it.
...
• WritableSearchPath — a PATH value, or a library search directory, names a directory that someone other than root or the owner of the declaring file can write (world-writable, group-writable for a group other than root's, or owned by another account), or a relative component that searches the current directory. The declaring file's owner is trusted because they could edit the path itself, so ~/bin in a user's own profile is not flagged. A sticky directory such as /tmp counts here, unlike for WorldWritable: a search path needs only a new name, and anyone can still create one there
...
The loop gates a release. It runs on every push to main, on tags and nightly, not on pull requests: nine to fourteen minutes an image is longer than a change should wait for its checks. A break it alone would catch is found after merging, and blocks the release until fixed.
Distro matrix
...
• What it runs is verified: its target is packaged and intact or Reproduced, or is a packaged directory, which has no digest to hold. Otherwise it would be the missing-md5sums case, one step removed.
• Its own file is packaged and intact or Reproduced (§7), or it is the package manager's own machinery: an rpm scriptlet read out of a package header, or a dpkg maintainer script. No package manager records a digest for its own metadata, so these can never be verified, and an ordinary Debian host carries hundreds. §7 already takes the database at its word about who owns every file, so hiding what the database itself holds adds no exposure. The exception is a maintainer script whose inode changed after its package was installed (§7): dpkg did not write that, and it is shown. The same holds for any entry carrying changed_after_install, which includes a setuid or setgid file on a dpkg host, live, whose inode changed well after its package's .list was written and whose mode dpkg-statoverride does not record: dpkg records no modes, so chmod u+s on a packaged binary otherwise verifies clean.
NSS modules have their own form of the third condition, because /etc/nsswitch.conf is unverifiable on every supported distribution: libnss-systemd's postinst edits the Debian template, and Fedora's authselect renders the file with no digest recorded. Where the file is not verified — not Reproduced (§7) — a module is hidden when the library it loads is packaged and intact, or when it has none and glibc skips the name; never when it follows a #.
...
• §5 and §7: only pam_exec lines and non-standard modules were entries, so a pam_unix.so replaced in the standard directory, and a line added to a generated stack, went unreported. Every rule line is an entry now, its module checked against its package, and the generated stacks are verified by regenerating them.
